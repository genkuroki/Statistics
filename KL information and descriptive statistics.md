...
2019-09-13～2019-09-23, 2019-10-15, 2020-01-03, 2020-11-02, 2025-10-17, 2026-08-27

* Copyright 2019, 2020, 2025, 2026 Gen Kuroki
...
__証明:__ $n$ が奇数の場合. $n=2m-1$ とおく. このとき, $t < x_m$ ならば

$$
f(t) = \sum_{k=1}^{m-1} (\pm(x_k-t)) + \sum_{k=m}^{2m-1} (x_k - t)
= (\text{負整数})\,t + (\text{定数})
$$
...
$$
f(t) = \sum_{k=1}^{m} (-x_k + t) + \sum_{k=m+1}^{2m-1}(\pm(x_k - t))
= (\text{正整数})\,t + (\text{定数}).
$$

ゆえに, $f(t)$ は $t < x_m$ で狭義単調減少し, $t > x_m$ で狭義単調増加する. ゆえに $f(t)$ が最小値になることと $t=x_m$ は同値になる.

$n$ が偶数の場合. $n=2m$ とおく. このとき, $t < x_m$ ならば

$$
f(t) = \sum_{k=1}^{m-1} (\pm(x_k-t)) + \sum_{k=m}^{2m} (x_k - t)
= (\text{負整数})\,t + (\text{定数})
$$
...
$$
f(t) = \sum_{k=1}^{m+1} (-x_k + t) + \sum_{k=m+2}^{2m}(\pm(x_k - t))
= (\text{正整数})\,t + (\text{定数})
$$

であり, $x_m\le t\le x_{m+1}$ ならば

$$
f(t) = \sum_{k=1}^{m} (-x_k + t) + \sum_{k=m+1}^{2m}(x_k - t)
= (\text{定数}).
$$

ゆえに, $f(t)$ は $t<x_m$ で狭義単調減少し, $x_m\le t\le x_{m+1}$ で一定になり, $t > x_{m+1}$ で狭義単調増加する. ゆえに $f(t)$ が最小値になることと $x_m\le t\le x_{m+1}$ は同値になる. $\QED$


__別証明:__ 一般に, 実数 $y\le z$ に関する函数 $g(t) = |y-t|+|z-t|$ の値は次のように計算される:
