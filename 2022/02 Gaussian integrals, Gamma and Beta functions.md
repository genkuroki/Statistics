...
本当は $z$, $t_i$ に $z = x_1+\cdots+x_{n+1}$, $t_i = x_i/z$ を代入した式を書きたかったのだが, 式が煩雑になり過ぎたのでそうしていない.  この式の $z$, $t$ にはそのように代入してから積分を行うという意味だと解釈する必要がある.  そのとき
