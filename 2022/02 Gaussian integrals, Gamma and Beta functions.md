...
plot(x, x -> p(n, x); label="Gamma(n+1,1)")
...
__注意:__ 以上のグラフにおけるガンマ分布と正規分布の密度函数の差の大部分は $x-n$ に関する奇函数になっている.  積分するとその奇函数成分は消える.
