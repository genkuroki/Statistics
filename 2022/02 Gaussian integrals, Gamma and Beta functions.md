...
* 黒木玄
* 2022-04-10～2022-06-30, 2024-01-06, 2025-05-20, 2026-05-12
$
...
\cdots = (\alpha+k-1)(\alpha+k-2)\cdots(\alpha+1)\alpha\Gamma(\alpha)
$$
...
ベータ分布の密度函数は $0 < \alpha < 1$ ならば $t\searrow 0$ のとき無限大になり, $0 < \beta < 1$ ならば $t\nearrow 1$ のとき無限大に発散することには注意しなければいけない.  

__$\alpha$ と $\beta$ の両方が大きいとき, ベータ分布は正規分布で近似される.__
...
ベータ分布 $\op{Beta}(\alpha, \beta)$ の期待値と分散がそれぞれ
...
__注意:__ ベータ分布 $\op{Beta}(\alpha, \beta)$ の期待値を
...
この式から, 期待値 $p$ を一定にしたまま $\alpha, \beta$ を大きくすると分散が小さくなることが分かる.
...
$u = x/(\nu/2)$ とおくと, $du/u = dx/x$ (一般に $u=ax$ のとき $du=a\,dx$ なので両辺を $u=ax$ で割ると右辺で分子分母の $a$ がキャンセルして $du/u=dx/x$ を得る) なので,
