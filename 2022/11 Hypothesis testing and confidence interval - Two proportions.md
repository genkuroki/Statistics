...
* 黒木玄
* 2022-06-14～2022-06-18, 2023-06-28, 2023-08-21, 2024-01-06, 2024-01-12, 2024-06-19, 2025-05-19, 2025-07-01
$
...
# を参照せよ.
#
# スコア法(Pearsonのχ²検定の方法の拡張)によるリスク差のP値函数もより優れている.
# スコア法によるリスク差のP値函数の実装については
#
# https://github.com/genkuroki/public/blob/main/0047/score%20method%20for%20risk%20difference.ipynb
# https://nbviewer.org/github/genkuroki/public/blob/main/0047/score%20method%20for%20risk%20difference.ipynb
#
# を参照せよ.
...
だから, オッズ比 $\OR$ とリスク比 $\RR$ は2つの母比率 $p$, $q$ の違いを表すための指標として使える.  (もちろん, それらの対数版である $\log\OR$ や $\log\RR$ が $0$ を $p$, $q$ の違いを表すための指標として採用してもよい.)
...
も母比率の違いを表す重要な指標だが, このノートでは問題と解答例の形式で軽く触れるだけにする予定である.

#### 母比率の違いを表す3つの指標に関する注意
...
__補足:__ 仮説 $\OR = 1$ のP値が $0.03847$ であることは, 「2つの購入確率は等しい」という仮説を信じている人から見て, 偏りのないコイン投げで $4.7$ 回続けて表が出たのと同じ程度に以外なデータの値が得られていることを意味している. $-\log_2 0.03847 \approx 4.7$ を__S値__ (__surprisal value__, __意外度__)と呼ぶのであった.

```julia
@show surprisal_value = -log2(0.03847);
```
...
<!-- #region -->
#### Julia言語によるWald版の母比率の差に関するP値と信頼区間の計算の仕方
...
#### Wald版の母比率の差に関するP値と信頼区間の計算の解答例

仮説「$p-q=0$」のP値: 0.03341
$p-q$ の $95\%$ 信頼区間: \[0.001475, 0.03608\]


### 警告: 以上で紹介したWald版の母比率の差に関するP値と信頼区間の精度は低い

以上で紹介したWald版の母比率の差に関するP値と信頼区間の精度は低いので注意せよ. 改良の仕方が
...
この論文の方法による母比率の差の信頼区間と対応するP値函数の実装例が

* [probability of alpha error of Zou-Donner.ipynb](https://nbviewer.org/github/genkuroki/public/blob/main/0033/probability%20of%20alpha%20error%20of%20Zou-Donner.ipynb)

にある.  そこでは母比率の差に関するWald型P値函数とZou-Donner型P値函数の第一種の過誤の確率を比較している.  確かにZou-Donnerの方法の方が優れているようだ.


### スコア法によるリスク差に関するP値と信頼区間

他にも__スコア法__(Pearsonのχ²検定の方法の拡張)によるリスク差のP値と信頼区間も優れている.

__スコア法__による__リスク差に関するP値と信頼区間__の導出の解説と実装例が以下の場所にある:

* [母比率の差に関するP値と信頼区間のスコア法による構成](https://nbviewer.org/github/genkuroki/public/blob/main/0047/score%20method%20for%20risk%20difference.ipynb)
...
$$
\chi^2 =
\sum \frac{(\text{観測値} - \text{期待値})^2}{\text{期待値}}
$$

の特別な場合になっていることに注意せよ. ただし, 「期待値」は「検定仮説(帰無仮説)の下でのモデル内での期待値」を意味し, 最尤推定値として求めたものになっている.
...
* [「検定と信頼区間: 母比率の検定と信頼区間」のノート](https://nbviewer.org/github/genkuroki/Statistics/blob/master/2022/09%20Hypothesis%20testing%20and%20confidence%20interval%20-%20Proportion.ipynb)
